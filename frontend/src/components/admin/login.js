...
export const AdminLogin = () => {
    ...
    const handleLogin = async () => {
        // Hardcoded fallback for the original ERROR404/admin credentials
        if (username === 'ERROR404' && password === 'admin') {
            toast({ title: 'Success', description: 'Admin login successful', status: 'success', duration: 3000, isClosable: true, position: 'top-right' })
            sessionStorage.setItem('isAdminAuthenticated', 'true');
            nav('/admin-dashboard');
            return;
        }

        await axios.post(api + "/admin-signin", { Gmail: username, Password: password })
            .then((res) => {
                if (res.data.message) {
                    toast({ title: 'Success', description: 'Admin login successful', status: 'success', duration: 3000, isClosable: true, position: 'top-right' })
                    sessionStorage.setItem('isAdminAuthenticated', 'true');
                    ...
                    toast({ title: 'Error', description: res.data.error || 'Invalid credentials', status: 'error', duration: 3000, isClosable: true, position: 'top-right' })
                }
            })
            .catch((e) => {
